...
namespace exec {
    ...
    template<>
    struct details::impls_for<continues_on_t> : default_impls {
        template<typename SigT>
        using as_tuple_t =
            completion_args_of<SigT>::template apply<
                meta_bind_front<decayed_tuple,
                                completion_tag_of_t<SigT>>::template type>;
        ...
        [[nodiscard]] static consteval auto get_completion_signatures() noexcept {
            using child_sender_t = child_of_t<SenderT>;
            using schedule_t = schedule_result_t<data_of_t<SenderT>>;
            using child_completion_signatures_t =
                conditional_meta_apply_t<
                    sends_stopped<schedule_t, EnvTs...>,
                    meta_bind_front<meta_merge_t, completion_signatures_of_t<child_sender_t, EnvTs...>>::template type,
        ...
        static constexpr auto get_state =
            ...
            {
                struct state {
                    using impls = impls_for<continues_on_t>;
